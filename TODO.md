...
- [x] 認証状態の永続化（Zustand persist + localStorage）
- [x] リロード時の認証状態復元（isHydrated対応）
...
- [x] **バックエンド**
  - [x] カタログCRUD API（GET/POST/PATCH/DELETE）
  - [x] カテゴリ管理（8カテゴリ対応）
  - [x] 組織別アクセス制御
  - [x] ページネーション・検索・フィルター
  - [ ] 画像アップロード（S3互換ストレージ）
  - [ ] 仕様書PDF管理
- [x] **フロントエンド**
  - [x] カタログ一覧ページ（@tanstack/react-table）
  - [x] カタログ登録/編集フォーム（shadcn/ui form + zod）
  - [x] カテゴリフィルター
  - [x] 検索機能
  - [x] ソート機能
  - [ ] カタログ詳細ページ
...
- [x] **バックエンド（pytest）**
  - [x] PostgreSQL テストデータベース構成
  - [x] 認証サービステスト（15テスト）
  - [x] カタログサービステスト（15テスト）
  - [x] セキュリティユーティリティテスト（17テスト）
  - [x] 認証APIテスト（14テスト）
  - [x] カタログAPIテスト（17テスト）
  - [x] 現在のカバレッジ: 90%
- [ ] **フロントエンド（vitest）**
